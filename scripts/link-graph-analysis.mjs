...
function analyzeHubPages() {
  ...
  Object.entries(hubLinkMatrix).forEach(([, targets]) => {
...
function generateReport() {
  ...
  const leastLinked =
    leastLinkedLinked.length > 0
      ? leastLinkedLinked[leastLinkedLinked.length - 1]
      : sortedInbound[sortedInbound.length - 1];
  ...
  // Outbound analysis
  const outboundCounts = Object.entries(hubLinkMatrix)
    .map(([hub, targets]) => ({
      hub,
  ...
  console.log(
    `  ${outboundCounts[0].hub}: ${outboundCounts[0].count} links → ${hubLinkMatrix[outboundCounts[0].hub].join(', ')}\n`
  );

  console.log(`Least outbound links:`);
  const leastOut = outboundCounts[outboundCounts.length - 1];
  console.log(
    `  ${leastOut.hub}: ${leastOut.count} links → ${hubLinkMatrix[leastOut.hub].join(', ')}\n`
  );
